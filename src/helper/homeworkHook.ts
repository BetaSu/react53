import {log, getType2Use, exist, createCounter} from './utils';
...
bindHook(3, 'beginWork', (_, wip) => {
  log('green', `创建${getType2Use(wip)}对应fiberNode`);
...
bindHook(3, 'commitPlacement', (fiber) => {
  log('brown', `执行副作用：${getType2Use(fiber)}插入`, fiber);
})
bindHook(3, 'commitWork', (fiber) => {
  log('brown', `执行副作用：${getType2Use(fiber)}更新属性`, fiber);
})
bindHook(3, 'commitDeletion', (fiber) => {
  log('brown', `执行副作用：${getType2Use(fiber)}删除DOM`, fiber);
})
